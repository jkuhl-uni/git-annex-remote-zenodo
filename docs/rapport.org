...
ne peut pas faire un puzzle sans avoir toutes ls pièces
...
l’on peut publier à la fin pour archiver ces fichiers. De l’autre
...
initialiser plusieurs remote dans un répertoire de fichiers et on peut
choisir quels fichiers à stocker sur quel remote, et lesquels à
laisser en local. Donc pour faire le mapping git-annex / Zenodo, on
avait deux possibilités: soit avoir une implémentation bijective
1-to-1 où on associe chaque dépôt Zenodo à un remote, ou une relation
surjective où l’utilisateur choisit le nombre de dépôt Zenodo à lier à
un seul remote.
