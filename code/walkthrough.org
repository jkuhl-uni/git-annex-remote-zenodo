...
* Preparing for the remote.
1. First, let's make the directory where we will be doing our tests.
...
2. Now, we have to initialize the git and the git annex directory before
...
3. Lastly, and in order to manipulate a git-annex remote, we will need to
...
2. Now, we can simply work with git annex like we would usually
...
#+begin_src
cd ~/test_remote
git annex initremote test-zenodo type=external externaltype=zenodo key=ACCESS_TOKEN encryption=none url=sandbox
#+end_src

* Manipulating files in the remote.
...
#+BEGIN_SRC shell :session *shell* :results output :exports both
git annex copy test1.txt --to test-remote
git annex copy test2.txt --to test-remote
git annex copy test3.txt --to test-remote
#+END_SRC
